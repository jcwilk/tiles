...
| `./rl` | Rate limit CLI (TypeScript). Usage, limits, alerts, and cost. Uses Wrangler KV and `GET /usage` from the worker. Requires worker running. `RATE_LIMIT_API_URL` (default: http://localhost:8787). `./rl cost` shows actual spend (needs `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`), estimated from counters, and max potential. Run `./rl help` for subcommands. |
